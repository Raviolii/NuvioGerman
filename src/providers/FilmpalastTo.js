...
// ==========================================
// 2. VOE DECODER (Updated with LaMovie Logic)
// ==========================================
...
async function extractVoe(url) {
    try {
        ...
        // 1. Find the encoded payload and the JS loader (Script Tag)
        ...
        // 2. Fallback to direct extraction if not obfuscated
        var re = /(?:mp4|hls|file)['"\s]*:\s*['"]([^'"]+)['"]/gi;
        var m;
        while ((m = re.exec(html)) !== null) {
            var candidate = m[1];
            if (candidate && candidate.indexOf("aHR0") === 0) return b64decode(candidate);
            if (candidate && candidate.indexOf("http") === 0) return candidate;
        }
    } catch (e) {
        console.log("[Filmpalast] VOE Error: " + e.message);
    }
    return null;
}

// ... (extractVidara remains the same as your previous working version)
async function extractVidara(urlStr) {
    try {
        ...
        var response = await fetch(apiBase + '/api/stream', {
            method: 'POST',
            headers: Object.assign({}, DEFAULT_HEADERS, { 'Content-Type': 'application/json', 'Referer': urlStr }),
